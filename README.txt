...
* **Behaviour change — bulk Enable All / Disable All scope.** Scope is still "every category on the active tab", but the tabs are different, so the blast radius is different. Five categories now legitimately span two groups (Block, Settings, Media, Cache, Database) and are in scope for both of their tabs. A category's enable toggle and All/Specific mode remain a single shared setting: turning "Block" off from the Appearance tab also removes its authoring abilities from the Blocks tab. Each card's membership chips show every tab it belongs to.
