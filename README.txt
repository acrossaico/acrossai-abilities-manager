...
* **Moved — 8 abilities relocated from `Content/` to `Block/`.** `blocks/add-block`, `blocks/remove-block`, `blocks/move-block`, `blocks/duplicate-block`, `blocks/update-post-block`, `blocks/get-post-blocks`, `blocks/outline-post-blocks` and `blocks/insert-pattern` registered under the `blocks/` namespace while declaring the `content` category. Their `category` is now `acrossai-abilities-manager-block` and their `sub_group` is `post-blocks`. **Ability slugs are unchanged, so no MCP client is affected.** If you had the Content category set to "Specific" mode with any of those eight ticked, those selections are keyed to the old category and will need re-ticking under Blocks.
* **Behaviour change — bulk Enable All / Disable All scope.** Scope is still "every category on the active tab", but the tabs are different, so the blast radius is different. Five categories now legitimately span two families (Block, Settings, Media, Cache, Database) and are in scope for both of their tabs. A category's enable toggle and All/Specific mode remain a single shared setting: turning "Block" off from the Appearance tab also removes its authoring abilities from the Blocks tab. Each card's membership chips show every tab it belongs to.
...
* **Deep links to retired tabs fall back to "All".** `?tab=core`, `?tab=media`, `?tab=themes`, `?tab=plugins`, `?tab=file-manager`, `?tab=comments`, `?tab=content-search`, `?tab=site-health`, `?tab=widgets` and `?tab=settings` no longer resolve. Invalid tab values were already handled silently — no error, no console warning.
