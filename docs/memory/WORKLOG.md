...
### 2026-09-10 — Feature 101: Ability Integrations tabs regrouped into 13 task groups; `core` retired; `tab_group` becomes per-ability

- **Why durable**: Establishes that tabs are **task groups named for the job**, not folder labels, and that the group count is now a *capped* number rather than a preference — Feature 100 turns each group into an always-loaded MCP tool, and Anthropic documents tool-selection accuracy degrading past 30–50 tools (production telemetry: Haiku below 90% between 10 and 15). One taxonomy now serves both the admin UI and the tool catalogue. See `DEC-ABILITY-GROUP-TAXONOMY`.
- **Future mistake prevented**: (1) **A bucket forms one copy-paste at a time.** `core` reached 105 abilities across nine folders by exactly the mechanism `PATTERN-ABILITY-LIBRARY-TAB-AUTO-DERIVE` warned about after the Elementor incident — the warning existed and the bucket still grew, because "copy the neighbour's tab_group" is the path of least resistance and nothing fails when it is wrong. The pattern is now backed by `tests/phpunit/Modules/Library/Test_Ability_Group_Map.php`, which holds the canonical map and names the misfiled ability. A documented rule with no failing test is a suggestion. (2) **Scope the impact analysis to the change, not to the field you started with.** The blast-radius survey was scoped to `tab_group` and correctly concluded it is display-only, unpersisted and unasserted by any test. Adding a *file move* late in planning changed `category` and the file's path — and 89 tests failed on hardcoded `Abilities/Content/X.php` source paths and `'acrossai-abilities-manager-content'` assertions. The survey was accurate about the field and silent about the operation. Re-run impact analysis when the operation changes, not just when the target does. (3) **The mechanism was already there.** `LibraryCard` had rendered "Tab membership" chips since Feature 055, so cards spanning several tabs were a designed behaviour — the Settings and Site Health half-cards were not a broken mechanism but unassigned data. Read the component before concluding a UI symptom is a bug.
- **Also shipped**: an objective test for relocating an ability's file — **move it only when its slug namespace already disagrees with its folder**. Eight abilities registering as `blocks/*` inside `Content/` qualified and moved to `Block/` with their slugs untouched, so no MCP client was affected. Three superficially similar candidates (`media/*-upload-mime-types`, `cache/flush-rewrite-rules`, `database/cleanup-expired-transients`) did not: their slug matches their folder, so moving them would either relocate the same inconsistency or force a slug rename, which README.txt records as breaking with no back-compat alias. Multi-purpose abilities are what per-ability `tab_group` is for.

---

### 2026-09-10 — Feature 101 follow-up: ability category slugs shortened to `acrossai-*`, with a two-store migration

- **Why durable**: Establishes that **an ability category slug is a persistence key, not a label**. It is the key of `acrossai_library_config` AND the `category` column of every DB-defined ability. WordPress refuses to register an ability whose category is not registered — `WP_Abilities_Registry::register()` calls `_doing_it_wrong()` and **returns null** — so renaming categories without migrating those stores makes operator-created abilities silently cease to exist, visible only under `WP_DEBUG`. Any future rename of a category slug is a data migration, full stop.
- **Future mistake prevented**: (1) **The sweep would have corrupted three unrelated strings.** Thirty distinct `acrossai-abilities-manager-*` strings exist; only 25 are categories. The other five are two asset handles registered in `admin/Main.php`, a CSS class in `admin/Partials/Menu.php`, a docblock referring to a *dropped* category, and a synthetic test fixture. This is the same class of false positive `DEC-SLUG-CONVENTION-VERB-FIRST` recorded for Feature 058 — a prefix that looks unique but is also used for handles and classes. **Enumerate every distinct match before any prefix sweep, and drive the rewrite from an explicit allow-list, never a regex.** The migration's `OWNED` constant is that list, and a test asserts it covers every category the plugin actually registers. (2) **`content` shadows `content-search`.** Applying the replacements in arbitrary order mangles the longer slug; sorting longest-first is required, and is now pinned by its own test. (3) **A coverage test can pass vacuously.** The first version of `test_owned_list_covers_every_registered_category` scanned for `'category' => '…'` only, so it found 24 of 25 — Rank Math declares `const CATEGORY = '…'` without quotes and was missed. The diff against `OWNED` was empty, so it passed. It now scans both declaration shapes and asserts a minimum discovery count, so an empty scan fails instead of looking clean.
- **Also shipped**: the migration runs from both `Activator::activate()` and `admin_init` priority 1, because an in-place upgrade (composer, wp-cli, file replace) never re-activates — activation-only migrations miss most real upgrades. Guarded by a completion flag so it costs one option read per admin request afterwards. Config rewriting prefers an entry already under the new key over a stale legacy one, so re-running cannot clobber a newer preference.

---

### 2026-09-10 — Feature 061's Debugging category was never registered; 7 abilities had never existed at runtime

- **Why durable**: A `Category_Registrar` that is written but never wired into `AcrossAI_Core_Abilities_Bootstrap::register_category_callbacks()` fails **silently and completely**. WordPress rejects every ability claiming an unregistered category — `WP_Abilities_Registry::register()` calls `_doing_it_wrong()` and returns null — so the abilities are constructed, attempt to register, and vanish. `includes/Abilities/Debugging/Category_Registrar.php` shipped with Feature 061 and was never added to the list; its seven conflict-testing abilities were absent for the feature's entire life. 399 notices accumulated in `debug.log` and nobody read them, because a `_doing_it_wrong` notice is invisible unless `WP_DEBUG_LOG` is on and someone looks.
- **Future mistake prevented**: **Registering abilities is two steps, and only one of them is obvious.** The ability files, their instantiation in the bootstrap, and the registrar file all existed and looked complete; the single missing line was in a different method of the same class. Guarded now by `tests/phpunit/abilities/Test_Category_Registrar_Wiring.php`, which asserts in both directions — every folder shipping a registrar is wired, and every folder whose abilities are instantiated has a registrar — plus a minimum-discovery assertion so a broken glob cannot make it pass vacuously. Verified by reverting the fix and watching it name `Debugging`.
- **Also shipped**: the bug explains a documentation mystery. `docs/abilities-inventory.md` recorded "389 abilities across 24 namespaces" and omitted Debugging entirely — whoever last updated it was reading runtime data, where those seven genuinely did not exist. The inventory is now generated from source (`scripts/generate-abilities-inventory.php`), which is why the discrepancy surfaced: source said 25 namespaces, runtime said 24. **A generated artifact disagreeing with a hand-maintained one is a signal, not a formatting nuisance.**

---

### 2026-09-12 — Feature 103: Contact Form 7 ability suite — 25 abilities for a host that ships none, verified by executing every one
