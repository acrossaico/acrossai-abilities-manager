<?php
/**
 * Feature 127 — the sole ability assembler for the UpdraftPlus suite.
 *
 ...
 * look interchangeable: UpdraftPlus schedules backups and restores them, All-in-One labels archives
 * and needs a paid extension to restore. Every real difference had to be reported as a capability
 ...
 */
abstract class Base_UpdraftPlus_Ability extends Ability_Definition {

	/**
	 * @since 0.0.35
	 * @var   string
	 */
	protected const CATEGORY = 'acrossai-updraftplus';

	/**
	 * @since 0.0.35
	 * @var   string
	 */
	protected const TAB_GROUP = 'updraftplus';
	...
	protected function ability(): array {
		...
		return array(
			...
			'args' => array(
				...
				'permission_callback' => UpdraftPlus_Guard::can( $this->permission_floor() ),
				'input_schema'        => array(
					'type'                 => 'object',
	...
	public function execute( array $input = array() ): array {
		$available = UpdraftPlus_Guard::assert_available();

		if ( is_wp_error( $available ) ) {
			return UpdraftPlus_Guard::fail( $available );
		}

		if ( $this->needs_confirmation_for( $input ) ) {
			$confirmed = UpdraftPlus_Guard::assert_confirmed( $input, $this->confirmation_message() );

			if ( is_wp_error( $confirmed ) ) {
				return UpdraftPlus_Guard::fail( $confirmed );
			}
		}
		...
		if ( is_wp_error( $result ) ) {
			return UpdraftPlus_Guard::fail( $result );
		}

		$message = isset( $result['message'] ) ? (string) $result['message'] : $this->success_message();
		unset( $result['message'] );

		return UpdraftPlus_Guard::ok( $result, $message );
	}
}
