 ...
 * @since 0.0.35
 */
final class Delete_Backup extends Base_UpdraftPlus_Ability {
	...
	protected function slug(): string {
		return 'updraftplus/delete-backup';
	}
	...
	protected function run( array $input ) {
		$result = Backup_Repository::delete_backup( (string) $input['id'] );
		...
		$remaining = Backup_Repository::status();
		...
		if ( 0 === $left ) {
			$result['note'] = trim(
				...
				. __( 'That was the last backup set UpdraftPlus held. There is now nothing to restore this site from.', 'acrossai-abilities-manager' )
			);
		}
