...
- **FR-005**: System MUST refuse a write when `sanitize_filename_check` is true AND `sanitize_file_name(basename) !== basename`, returning `blocked_reason:'filename_sanitize_failed'` with the original `input` and WordPress-`sanitized` variant in the response.
