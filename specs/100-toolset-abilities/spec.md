...
**Created**: 2026-09-10
**Status**: Draft
**Input**: User description: "Add one Toolset dispatcher ability per ability group. A Toolset is a single ability that takes an `action` of discover | info | execute and dispatches to the abilities in its own group, so an AI assistant works through ~450 abilities one group at a time instead of receiving the whole catalogue in one response."

> **Grouping key changed from category to group (2026-09-10).** This specification originally grouped
> Toolsets by ability *category* — 25 of them. Feature 101 introduced **groups**: 13 task-shaped
> groups assigned per ability, which are what the Integrations screen's tabs now show. Toolsets follow
> groups, so there are 13, not 25. That is not a cosmetic swap; see *What the change to groups
...
A **Toolset** is one entry point per group. It answers three questions about its own group only:
...
Toolset, asks what it contains, and receives that group's abilities — not four hundred and fifty. For
a large group it can narrow by search term, by the card an ability belongs to, or by sub-group, and
...
exist. It returns that group's abilities and nothing from any other group.

**Acceptance Scenarios**:

1. **Given** a group holding seven abilities, **When** an assistant requests its contents, **Then**
   it receives those seven and no ability belonging to another group.
2. **Given** a group holding seventy-one abilities and no page size specified, **When** an assistant
...
3. **Given** the same group, **When** an assistant supplies a search term, **Then** it receives only
   entries whose name, label or description contains it, case-insensitively.
4. **Given** a group drawing on several cards, **When** an assistant supplies a card name, **Then**
   it receives only entries from that card.
5. **Given** a group whose abilities carry sub-groups, **When** an assistant supplies a sub-group,
...
**Independent Test**: Ask a Toolset about one ability in its group by name. It returns that ability's
...
1. **Given** an ability in the Toolset's group, **When** an assistant asks about it by name, **Then**
...
3. **Given** a request naming an ability that exists but belongs to a different group, **When** it is
   answered, **Then** the request is refused with a machine-readable reason distinguishing "not in
   this group" from "does not exist".
...
1. **Given** an ability in the Toolset's group and valid parameters, **When** an assistant runs it
...
5. **Given** a request naming an ability outside the Toolset's group, **When** it is answered,
...
2. **Given** a group whose every contributing card is switched off, **When** an assistant lists
   available tools, **Then** that group's Toolset is absent.
...
5. **Given** no extension is present, **When** an assistant opens a Toolset, **Then** it sees every
   registered ability in that group.

---

### User Story 5 - Groups added later are never silently missing (Priority: P3)

When an integration is activated, its group gains a Toolset without anyone editing this feature.
Groups with a purpose-written Toolset use it; anything else falls back to one generated from the
group's own name.
...
1. **Given** an integration is activated whose abilities form their own group, **When** an assistant
   lists available tools, **Then** a Toolset for that group is present.
2. **Given** that integration is deactivated, **When** an assistant lists available tools, **Then**
   that Toolset is absent.
3. **Given** a group with no purpose-written Toolset, **When** its fallback Toolset is created,
   **Then** it takes its name from that group's own identifier.
4. **Given** any group running on the fallback, **When** the site is in debug mode, **Then** a
   diagnostic names that group so the gap remains visible.

---

### Edge Cases

- **A group has no registered abilities.** No Toolset is created. An entry point that can only ever
  return an empty list is noise in the catalogue and misleads an assistant into thinking the subject
  area exists.
- **A group has abilities, but all are hidden.** The Toolset exists and returns an empty list with an
...
- **A requested ability does not exist anywhere.** Reported as not found, distinctly from "exists but
  not in this group" and from "exists here but hidden".
...
- **A card contributes to two groups.** Its abilities appear in whichever Toolset matches each
  ability's own group, never in both.
...
- **An extension narrows visibility per connection.** Two connections configured to expose different
  sets receive different listings from the same member list, because visibility is applied by the
  caller, never inside the lookup.
...
- **FR-001**: The system MUST provide exactly one Toolset per group that has at least one registered
  ability.
- **FR-002**: The system MUST NOT provide a Toolset for a group with no registered abilities.
- **FR-003**: Each Toolset MUST be identified by a name derived from its group, unique across the
...
- **FR-005**: Each Toolset MUST carry a human-readable label and a description stating which group it
  covers and which actions it supports.
- **FR-006**: The system MUST provide a purpose-written Toolset for each group shipped with this
  product, and MUST generate a fallback Toolset from the group's own identifier for any other group.
- **FR-007**: The system MUST emit a diagnostic, when the site is in debug mode, naming every group
...
- **FR-008b**: An individual Toolset MUST declare only its own identity: which group it covers, its
...
- **FR-008c**: The registration of the Toolsets' shared category, the published catalogue of Toolsets,
  and the fallback that covers a group with no purpose-written Toolset MUST also live with the shared
  definition rather than in separate components, so that adding a group means adding one declaration
  and nothing else.

**Grouping**

- **FR-009**: The system MUST group abilities into Toolsets by **group**, the per-ability grouping the
  Integrations screen's tabs display.
- **FR-010**: The system MUST NOT group Toolsets by ability category. A group may draw on several
  cards, and several groups may draw on one card.
- **FR-011**: The system MUST NOT alter the Integrations screen's existing tabs, cards or grouping.

**Discovering**

- **FR-012**: A Toolset MUST, on request, list the abilities in its group, returning for each the
...
- **FR-015**: A listing MUST accept a card name and return only entries drawn from that card. This is
  the primary narrowing axis for a large group, which may span five cards.
...
- **FR-022**: Description MUST apply the same group-membership and visibility rules as execution. An
  ability that may not be run through a Toolset MUST NOT have its schema disclosed by it.

**Executing**

- **FR-023**: A Toolset MUST, on request, run a named ability in its group with caller-supplied
...
- **FR-031**: Recoverable failures — unknown ability, ability outside this group, hidden ability,
...
- **FR-035**: A Toolset MUST include only abilities in its own group that are intended as callable
...
- **FR-040**: The system MUST publish a catalogue of the Toolsets it has created — group, label, name
  and member count — so other components can consume it without inspecting internals.

**Resolving the member list**

- **FR-040a**: The system MUST provide a single reusable way to ask which abilities belong to a given
  group, returning them in a stable order. Every Toolset MUST obtain its members through it, and it
  MUST be usable by anything else that needs the same answer.
- **FR-040b**: That lookup MUST return only facts derived from **registration** — which abilities
  exist and which group each belongs to. It MUST NOT apply visibility or permission decisions;
  those belong to the caller, because they vary between requests.
- **FR-040c**: The lookup MUST be the only place group membership is derived, so that a later change
  to how membership is resolved — including storing the answer between requests — happens in one
  place and changes no caller.

> **Deferred: storing the resolved list between requests.** An earlier revision of this specification
> required caching each group's member list, with an enumerated set of invalidation triggers. That is
> **out of scope for now** and will be specified when it is implemented. FR-040c exists so adding it
> later is a change behind one seam rather than a change to every Toolset.
>
> Two constraints are recorded here so they are not rediscovered:
>
> 1. **Only registration-derived facts may ever be stored.** The visibility extension point resolves
>    from the context of the call — the equivalent filter already shipped in the connected transport
>    takes the identifier of the connection handling the current request — so a stored post-filter
>    listing would serve one connection's view of the catalogue to another. A disclosure failure, not
>    a stale cache.
> 2. **Invalidation, not expiry, is the hard part.** What registers changes when this plugin is
>    activated or deactivated, when *any* plugin is, when an update completes, when the Integrations
>    settings are saved, when an ability is created or deleted, when an override changes, and when the
>    active site changes. Saving the Integrations settings currently announces nothing, so that
>    trigger has no hook to attach to — see FR-040d.
- **FR-040d**: Saving the Integrations settings MUST announce itself. Card on and off, the choice
  between all and specific, and the per-ability selection all change which abilities register, and
  nothing currently signals it. This is required independently of any caching, because it is the only
  way another component can react to the setting at all.
...
- **Toolset**: One entry point representing a single group. Has a stable name derived from its
  group, a label, a description, and three actions. Owns no data; its contents are derived on demand.
- **Group**: The per-ability grouping introduced by Feature 101 and displayed as the Integrations
  screen's tabs. Thirteen exist; two appear only while their integration is active. One group yields
  at most one Toolset.
- **Card**: The per-folder grouping a group draws on, and the unit a site owner enables or disables.
  A group may span several cards; a card may contribute to two groups. Serves as a narrowing axis
  inside a listing.
- **Member Ability**: A registered, callable ability whose group matches a Toolset's, which is not
...
## What the change to groups settles

Grouping by group rather than category is not a rename. It removes a problem the category-based draft
...
A group Toolset shares a category with nothing. It declares its own (FR-008), and resolves its members
...
**The card becomes a narrowing axis rather than a boundary.** The largest group holds seventy-one
...
- **SC-002**: No single listing response returns more than two hundred entries, and the default
  response returns at most fifty, regardless of how large the group is.
- **SC-003**: Listing the largest group returns at most fifty entries by default, against the roughly
...
- **SC-004**: An assistant can locate a named capability within a group in one request, by search term
  or by card, without paging through the whole group.
...
- **SC-009**: Every group present on the site is covered by exactly one Toolset, with no group left
  without one and no group served by two.
...
- **SC-012**: The combined size of the tool catalogue an assistant receives is recorded before and
  ...
  than assumed.
- **SC-013**: Group membership is derived in exactly one place, so a caller asking which abilities
  belong to a group and a Toolset listing its own members can never disagree.

## Assumptions

- **Groups are the right grouping, and this is a decision rather than an inference.** Groups are
  ...
  measurably degrades. Feature 101 established groups for the admin interface; using the same
...
- **Resolving members from what is registered is what makes the site owner's settings inherited rather
  ...
  ability registers at all. A Toolset that asks "what is registered in my group" gets all of that for
  free, including the inverted default that leaves an untouched integration card off.
- **The default is that a Toolset sees everything registered in its group.** This is broader than the
...
- **Descriptions are written by hand for the groups shipped with this product**, because the
...
- **Deriving a group's members is cheap, so storing it between requests is deferred rather than
  urgent.** Resolving a group filters a few hundred already-loaded ability objects. The value of a
  single lookup is that it removes duplication and gives a later optimisation one place to land, not
  that it saves measurable time today.
- **Numbers cited are current live counts** — thirteen groups over roughly 450 abilities, the largest
  group holding 71 across five cards — and are expected to grow. No requirement depends on a specific
  count.

## Dependencies

- **Feature 101** (merged as `5cf50ec4`) established groups, without which this feature has no
