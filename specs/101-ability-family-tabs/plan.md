# Implementation Plan: Ability Family Tabs

**Branch**: `101-ability-family-tabs` | **Date**: 2026-09-10 | **Spec**: [spec.md](./spec.md)
...
task families, assigning `tab_group` per ability rather than per folder. Shorten category slugs from
...
| `LibraryCard` renders "Tab membership" chips (Feature 055) | Cards spanning families are a designed behaviour |
...
| III. User-Centric Design | Pass — tabs named for tasks; split cards name every family they belong to |
...
| VI. Reusability & DRY | Pass — the family map lives once, in the test that pins it; the migration's owned list lives once and is verified against source |
...
tests/phpunit/
  Modules/Library/Test_Ability_Family_Map.php        new — the canonical map
...
### Family assignment is per ability, not per category

Per-category was considered first and rejected. It would have forced whole categories into one family,
...
component already displays each card's full family membership — without that, half a card would appear
in a tab with nothing indicating the rest existed, which was precisely the pre-existing Settings and
Site Health defect.

**Cost, accepted:** the enable toggle is keyed by category, so a card spanning two families shows one
...
which `README.txt` records as breaking with no compatibility alias. They were handled by family
